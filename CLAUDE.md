...
The six `res/icons/*.ico` are final art: a battery outline with one, two, or three fill bars for
efficiency, balanced, and performance, monochrome so the glyph follows the taskbar theme rather than
the mode. Treat them as source. Any replacement keeps the file names and the 16/20/24/32 frame set.
